...
export default function Home() {
  ...
  if (userAttempts.timestamp && new Date(userAttempts.timestamp).getTime() > now.getTime()) {
    setMessage('Too many attempts for this email. Try again after 24 hours.');
    setTimeout(() => {
    ...
    <div className={styles.container}>
          ...
          <div className={styles.popup1}>
            ...
            <form onSubmit={login}>
              <input
                ...
                required className={styles.input1}
              />
              <input
                type="password"
              ...
              <p style={{ marginTop: 10, textAlign: 'center' }}>
                <span onClick={() => { setShowLogin(false); setShowRegister(true); }} style={{ color: '#0070f3', cursor: 'pointer' }}>
                  Create New Account
                </span>
              </p>
              <p style={{ textAlign: 'center' }}>
                <span onClick={() => { setShowLogin(false); setShowForgotPassword(true); }} style={{ color: '#0070f3', cursor: 'pointer' }}>
                  Forgot Password?
                </span>
              </p>
            </form>
        ...
        <>
          ...
          <div className={styles.popup1}>
            ...
            <form onSubmit={register}>
              <input type="text" placeholder="Name" value={registerForm.name}
                onChange={(e) => setRegisterForm({ ...registerForm, name: e.target.value })} required className={styles.input1} />
              <input type="email" placeholder="Email" value={registerForm.emailId}
                onChange={(e) => setRegisterForm({ ...registerForm, emailId: e.target.value })} required className={styles.input1} />
              <input type="text" placeholder="Mobile Number" value={registerForm.mobileNumber}
                onChange={(e) => setRegisterForm({ ...registerForm, mobileNumber: e.target.value })} required className={styles.input1} />
              <input type="password" placeholder="Password" value={registerForm.password}
                onChange={(e) => setRegisterForm({ ...registerForm, password: e.target.value })} required className={styles.input1} />
      ...
      {showForgotPassword && (
        <div style={{
          ...
          <div style={{
            background: 'linear-gradient(to bottom, #f7941d, #ffffff 30%, #009245)',
            borderRadius: '20px',
            padding: '30px',
            width: '90%',
            maxWidth: '400px',
            ...
            <form onSubmit={handleForgotPassword}>
              <input type="email" placeholder="Enter your email" value={email}
                onChange={(e) => setEmail(e.target.value)} required style={{
                  ...
                  boxShadow: 'inset 0 0 5px rgba(0,0,0,0.2)', fontSize: '16px'
                }} />
