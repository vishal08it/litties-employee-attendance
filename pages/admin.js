...
export default function Admin() {
  ...
  const handleAddEmployee = async (e) => {
    e.preventDefault();
    let imageUrl = '';
    if (image) imageUrl = await uploadImageToCloudinary();
  ...
  const generatePDF = async () => {
    ...
    for (const key in groupedData) {
      ...
      const rows = records.map((a) => {
        ...
        return [a.date, a.empId, a.name, punchIn, punchOut, timeDiff, dayType];
      });
